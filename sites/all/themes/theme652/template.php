...
/**
 * @file
 * Contains theme override functions and process & preprocess functions for theme652
 */

// Auto-rebuild the theme registry during theme development.
if (theme_get_setting('theme652_clear_registry')) {
  // Rebuild .info data.
 ...
 */
function theme652_html_head_alter(&$head_elements) {
 ...
 */
function theme652_preprocess_search_block_form(&$vars) {
 ...
 */
function theme652_preprocess(&$vars, $hook) {
  $vars['theme652_path'] = base_path() . path_to_theme();
}

/**
 * Implements template_preprocess_html().
 */
function theme652_preprocess_html(&$vars) {
  
  $vars['doctype'] = _theme652_doctype();
  $vars['rdf'] = _theme652_rdf($vars);
  ...
  if (!$vars['is_front']) {
    ...
    if (isset($page_name)) {
      $vars['classes_array'][] = theme652_id_safe('page-' . $page_name);
    }

    $vars['classes_array'][] = theme652_id_safe('section-' . $section);
 ...
 */
function theme652_preprocess_page(&$vars) {
 ...
 */
function theme652_preprocess_maintenance_page(&$vars) {
  ...
  drupal_add_css(drupal_get_path('theme', 'theme652') . '/css/maintenance-page.css');
}
 ...
 */
function theme652_preprocess_node(&$vars) {
  // Striping class
 ...
 */
function theme652_preprocess_block(&$vars, $hook) {
 ...
 */
function theme652_menu_tree($vars) {
 ...
 */
function theme652_field__taxonomy_term_reference($vars) {
  $output = '';
 ...
 */
function theme652_breadcrumb($vars) {
  
  $breadcrumb = isset($vars['breadcrumb']) ? $vars['breadcrumb'] : array();

// Determine if we are to display the breadcrumb.
  $show_breadcrumb = theme_get_setting('theme652_breadcrumb_display');
  if ($show_breadcrumb == 'yes') {
  
	  if (theme_get_setting('theme652_breadcrumb_hideonlyfront')) {
		$condition = count($breadcrumb) > 1;
	  } else {
		$condition = !empty($breadcrumb);
	  }
	  
	  if(theme_get_setting('theme652_breadcrumb_showtitle')) {
	  ...
	  $separator = theme_get_setting('theme652_breadcrumb_separator');
 ...
 * @return bool
 */
function theme652_tabs_float() {
  $float = (bool) theme_get_setting('theme652_tabs_float');
  $float_node = (bool) theme_get_setting('theme652_tabs_node');
...
function theme652_id_safe($string) {
  // Strip accents
 ...
 * @ingroup themeable
 */
function theme652_menu_link(array $vars) {
  ...
  $element['#attributes']['class'][] = theme652_id_safe($element['#title']);
 ...
 */
function theme652_preprocess_menu_local_task(&$vars) {
 ...
 */
function theme652_menu_local_tasks(&$vars) {
  $output = '';
